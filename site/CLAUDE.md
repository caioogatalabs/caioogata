...
Three modes via the `data-theme` attribute **on a section container** — all semantic tokens remap automatically:
...
The site itself is dark, full stop. A site-wide light skin — the attribute on `<html>`, a `DARK / LIGHT` switch, a saved preference — was built and taken back out on 2026-09-17: the page does not work in it. Do not rebuild it without deciding first what the hero, the covers and the header blend do there. `light` survives as a section value, which is what `/dev/buttons` previews; the footer is the only production section carrying one, and it carries `inverse`.
...
The blend needs the surface under it to be near one end of the range. Against the site's near-black it reads 16.8:1; against a mid grey it differences almost onto itself (1.5:1), which is one of the reasons the site-wide light skin came back out.
<!-- GSD:architecture-end -->

---

## On the GSD markers above

GSD is no longer used in this project (2026-09-10). The `<!-- GSD:*-start -->`
comments are vestigial — nothing regenerates these blocks any more, and the
`.planning/` sources they name are a historical record, not a live input.
This file is hand-maintained now: edit it directly.

The old workflow section required routing every edit through `/gsd:quick`,
`/gsd:debug` or `/gsd:execute-phase`. Those commands are not installed, so the
rule was unfollowable — it has been removed rather than left to trip up the
next agent.
