...
const COLOR_MAP: Record<string, string> = {
  ...
  success: 'text-text-brand',
  system: 'text-text-secondary',
  empty: 'text-text-tertiary opacity-20',
...
export function FormLog({
  ...
  useEffect(() => {
    if (isIdle && !idleStarted) setIdleStarted(true)
  }, [isIdle, idleStarted])
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setTypedLength((l) => l + 1), TYPEWRITER_MS)
    ...
    if (prevField && prevField !== focusedField) {
      ...
      if (currentVal && currentVal !== prevVal) {
        if (prevField === 'message') {
          const preview = currentVal.length > 40 ? currentVal.slice(0, 40) + '...' : currentVal
  ...
  useEffect(() => {
    ...
    for (const err of currentErrors) {
      if (!prev.includes(err)) pushLog(`[${getTimestamp()}] ✗ ${err}`, 'error')
    }
  ...
  useEffect(() => {
    ...
    if (currentErrors.length > 0) {
      ...
      for (const err of currentErrors) pushLog(`[${getTimestamp()}] ✗ ${err}`, 'error')
    }
  ...
  useEffect(() => {
    ...
    switch (status) {
      ...
      case 'error':
        ...
        if (errorRetry) pushLog(`[${getTimestamp()}] ${errorRetry}`, 'error')
        if (contactEmail) pushLog(`[${getTimestamp()}] → ${contactEmail}`, 'info')
        break
      case 'idle':
        if (prevStatusRef.current === 'success') pushLog(`[${getTimestamp()}] form reset`, 'system')
        break
    }
  ...
  if (isIdle) {
    const currentFullText = currentIdleIndex < allIdleMessages.length ? allIdleMessages[currentIdleIndex] : null
    ...
    if (typingText !== null) allVisibleMessages.push(typingText)
    ...
    for (let i = 0; i < emptyCount; i++) lines.push({ text: EMPTY_CHAR, type: 'empty', isCurrent: false })
    for (let i = 0; i < visibleIdle.length; i++) {
      lines.push({ text: `> ${visibleIdle[i]}`, type: 'idle', isCurrent: i === visibleIdle.length - 1 })
    }
      ...
      >
        {lines.map((line, i) => (
          <div key={i} className={COLOR_MAP[line.type]}>
            ...
            {line.isCurrent && (
              <span
                className={`inline-block w-1.5 h-3 bg-text-secondary align-middle ml-0.5 ${isTyping ? 'opacity-100' : 'animate-blink'}`}
                aria-hidden
              />
            )}
          </div>
  ...
  for (let i = 0; i < emptyCount; i++) paddedLines.push({ text: EMPTY_CHAR, type: 'empty' })
  for (const entry of visibleEntries) paddedLines.push(entry)
  ...
  return (
    ...
    >
      {paddedLines.map((line, i) => (
        <div key={i} className={`${COLOR_MAP[line.type] ?? COLOR_MAP.info} truncate`}>
          {line.text}
        </div>
      ))}
    </div>
