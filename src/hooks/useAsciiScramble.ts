...
interface UseAsciiScrambleOptions {
  enabled?: boolean
...
interface UseAsciiScrambleResult {
  chars: ScrambleChar[]
  isComplete: boolean
...
export function useAsciiScramble(
  finalText: string,
  options: UseAsciiScrambleOptions = {}
): UseAsciiScrambleResult {
  const {
    enabled = true,
    frameInterval = 16,
    charsPerFrame = 5,
  ...
  const resolved: UseAsciiScrambleResult = {
    chars: finalText.split('').map((c) => ({ char: c, locked: true })),
    isComplete: true,
    text: finalText,
  }

  const [result, setResult] = useState<UseAsciiScrambleResult>(() =>
